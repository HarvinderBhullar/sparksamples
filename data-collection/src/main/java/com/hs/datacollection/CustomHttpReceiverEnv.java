...
public class CustomHttpReceiverEnv extends Receiver<String> {

	private final String API_ROOT = "http://machinepark.actyx.io/api/v1";

	public CustomHttpReceiverEnv() {
	...
	{
		Client client = Client.create();
		WebResource webResource = client.resource("http://machinepark.actyx.io/api/v1/env-sensor");
		ClientResponse response = webResource.accept("application/json").get(ClientResponse.class);
		if (response.getStatus() != 200) {
			throw new RuntimeException("Failed : HTTP error code : " + response.getStatus());
		}
